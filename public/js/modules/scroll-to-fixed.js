define(['jquery', 'AX', 'ScreenTest'], function($, AX, ScreenTest) {
	'use strict';
	var log = AX.Util.log;
	var oConfig = {};
	var oDefaults = {
	...
	var fSetParentDomHeight = function(o$FixedDom){
		o$FixedDom.parent().height(oConfig.$container.height());
		o$FixedDom.parent().css('position', 'relative');	
	}

	var fResetParentDomHeight = function(o$FixedDom){
		o$FixedDom.parent().css({
			height: 'auto',
			position: 'static'
		});			
	}
	...
	var fFixToTop = function(o$FixedDom, nFixedTop) {
		...
		o$Win.on('scroll.fixtop', function() {
			...
			if(bIsOverFooter && ScreenTest.screen.bIsDesktopMode){
				fSetParentDomHeight(o$FixedDom);	
			}else{
				fResetParentDomHeight(o$FixedDom);
			}
